...
class Iiwa_pub_sub : public rclcpp::Node
{
    public:
          ...
          controller_(*robot_) 
        {
            ...
            // Declare trajectory_type parameter
            declare_parameter("trajectory_type", "linear_trapezoidal"); // Default trajectory
            get_parameter("trajectory_type", trajectory_type_);
            RCLCPP_INFO(get_logger(), "Selected trajectory type: '%s'", trajectory_type_.c_str());
            ...
            joint_velocity_old_.resize(nj);
            torque_values_.resize(nj);
            ...
            desired_commands_ = std::vector<double>(robot_->getNrJnts(), 0.0);
            ...
            // Plan trajectory
            double traj_duration = 1.5, acc_duration = 0.5, t = 0.0, traj_radius=0.1;
            
            // Initialize trajectories 
            if (trajectory_type_ == "linear_trapezoidal") 
            {
                    planner_ = KDLPlanner(traj_duration, acc_duration, init_position, end_position);
            } 
            else if (trajectory_type_ == "linear_cubic") 
            {
                    planner_ = KDLPlanner(traj_duration, init_position, end_position);
            }
            else if (trajectory_type_ == "circular_trapezoidal") 
            {
                    planner_ = KDLPlanner(traj_duration, acc_duration, init_position, traj_radius);
            } 
            else if (trajectory_type_ == "circular_cubic") 
            {
                    planner_ = KDLPlanner(traj_duration, init_position, traj_radius);
            } 
            else 
            {
                   RCLCPP_ERROR(get_logger(), "Invalid trajectory type selected!");
                   return;
            }
    ...
    private:
            ...
            if (t_ < total_time){
                ...
                if(cmd_interface_ == "position"){
                   ...
                   // joint_positions_cmd_ = joint_positions_;
                    ...
                    joint_positions_cmd_.data = joint_positions_cmd_.data + joint_velocities_cmd_.data*dt;
                }
                
                
                else if(cmd_interface_ == "effort"){
                    // Save current joint velocities
                    joint_velocity_old_.data = joint_velocities_.data;

                    // Compute desired cartesian velocities (assuming zero angular velocity)
                    Vector6d cartvel;
                    cartvel << p.vel, 0, 0, 0;
                    Vector6d cartacc; 
                    cartacc << p.acc + error/dt, 0,0,0;

                    // Compute desired joint velocities using inverse kinematics
                    joint_velocities_cmd_.data = pseudoinverse(robot_->getEEJacobian().data) * cartvel;
                    // Update desired joint positions
                    joint_positions_cmd_.data = joint_positions_cmd_.data + joint_velocities_cmd_.data * dt;
                    // Compute desired joint accelerations
                    joint_acceleration_d_.data = (joint_velocities_cmd_.data - joint_velocity_old_.data) / dt;


                    // Compute torque values using inverse dynamics controller
                    torque_values_ = controller_.idCntr(joint_positions_cmd_, joint_velocities_cmd_, joint_acceleration_d_, _Kp, _Kd);
                }
                ...
                if(cmd_interface_ == "effort"){
                    for (long int i = 0; i < torque_values_.size(); ++i) {
                        desired_commands_[i] = torque_values_(i);
                    }
                } else {
                    for (long int i = 0; i < joint_velocities_.data.size(); ++i) {
        ...
        KDL::JntArray joint_velocity_old_;
        Eigen::VectorXd torque_values_;
        KDLController controller_;
        double _Kp = 160.0;  // Example proportional gain
        double _Kd = 28.0;    // Example derivative gain
