...
void KDLPlanner::trapezoidal_vel(double t_, double tc_, double tf_, double &s, double &s_dot, double &s_dotdot) {
  // t = time, tc = acceleration time
  double sc_dotdot;
  sc_dotdot=(2/(tf_*tc_-std::pow(tc_,2)));
...
void KDLPlanner::cubic_polynomial(double t_, double tf_, double &s, double &s_dot, double &s_dotdot) {
    ...
    double s_0, s_final, v_0, v_final, a0, a1, a2, a3;
    ...
    a0 = s_0;
    a1 = v_0;
    a2 = (3 * (s_final - s_0) / std::pow(tf_, 2)) - ((2 * v_0 + v_final) / tf_);
    a3 = (-2 * (s_final - s_0) / std::pow(tf_, 3)) + ((v_0 + v_final) / std::pow(tf_, 2));
