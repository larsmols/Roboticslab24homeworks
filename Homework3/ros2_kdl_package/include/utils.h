...
inline Eigen::Matrix<double, 6, Eigen::Dynamic> toEigen(const KDL::Jacobian& jacobian) {
    Eigen::Matrix<double, 6, Eigen::Dynamic> eigenJacobian(6, jacobian.columns());
    for (unsigned int i = 0; i < jacobian.columns(); ++i) {
        eigenJacobian.col(i) = Eigen::VectorXd::Map(jacobian.data.data() + i * 6, 6);
    }
